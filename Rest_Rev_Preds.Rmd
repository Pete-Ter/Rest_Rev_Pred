...
```{r runRF}
rFGrid <-  expand.grid(mtry=2:4, maxnodes=seq(5,14), nodesize=seq(6,11), 
...
```{r crossval}
mean.met <- CrossVal(train, 'revenue.log', as.formula(rf.form), 10, 2, rFGrid)
...
test <- test.pr
test$Prediction <- exp(test$score_rf)
test$Prediction
```
